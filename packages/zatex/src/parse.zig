...
pub const ParseCtx = struct {
    ...
    fn findDef(self: *ParseCtx, name: []const u8) ?*Def {
        ...
        while (i > 0) {
            i -= 1;
            if (!self.defs[i].is_char and tokNameEq(self.defs[i].name, name)) return &self.defs[i];
        }
        return null;
    }
