...
// zatex-png: LaTeX -> PNG screenshots. macOS-only (CoreGraphics
// backend); the layout core stays portable. See README.md.
pub fn build(b: *std.Build) void {
    ...
    // Coordinate-mapping tests for the CoreGraphics backend. Separate
    // root (per core convention) so the backend's tests run under the
    // same `test` step; needs the same frameworks to link.
