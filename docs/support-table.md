# ZaTeX KaTeX support-table copy (issue #14)

Pinned KaTeX: **0.18.7** (`tools/katex/package.json`; oracle for the sweep).
Upstream: https://katex.org/docs/support_table.html — refresh this copy
when the pin in #11 bumps, and open gap issues for new rows.

Legend: accept = differential golden vs pinned KaTeX (structure per
`docs/tolerance.md`); err-parity = reject agreement incl. error position;
div = declared divergence (`katex_only`, ours recorded);
unsup = KaTeX rejects it too; no accept row covers it (any accept row would fail the sweep), dedicated reject row only where listed;
TODO = not yet supported, owner = implementing issue.
A `*` after a golden id marks a divergence row (partial credit only).
Divergence rows: deep-nest, maxexpand-loop, text-dollar, text-math, verb-in-macro.
...
| `\%` | accept | goldens: sym-escapes |
| `&` | accept | goldens: aligned, alignedat, array |
| `\&` | accept | goldens: sym-escapes |
...
| `\(…\)` | accept | goldens: bigl, demo-cfrac, demo-sumsq |
| `\` | TODO | owner #14 |
| `\"` | accept | goldens: text, textaccent |
| `\$` | accept | goldens: sym-escapes |
...
| `{alignat\*}` | accept | goldens: aligned, alignedat, array |
| `{alignedat}` | accept | goldens: aligned, alignedat, array |
...
| `\argmax` | TODO | owner #1 |
| `\argmin` | TODO | owner #1 |
| `{array}` | accept | goldens: aligned, alignedat, array |
| `\array` | unsup | KaTeX rejects too; no reject row yet (owner #6) |
| `\arraystretch` | accept | goldens: aligned, alignedat, array |
...
| `\begin` | accept | goldens: aligned, alignedat, array |
...
| `\Braket` | accept | goldens: bigl, demo-cfrac, demo-sumsq |
| `\brace` | TODO | owner #2 |
...
| `{cases}` | accept | goldens: aligned, alignedat, array |
...
| `\cr` | accept | goldens: aligned, alignedat, array |
...
| `{dcases}` | accept | goldens: aligned, alignedat, array |
...
| `\displaystyle` | accept | goldens: demo-cauchy, demo-sumsq, displaystyle |
...
| `\dotsb` | TODO | owner #1 |
| `\dotsc` | TODO | owner #1 |
| `\dotsi` | accept | goldens: demo-fourier, demo-gauss, int |
| `\dotsm` | TODO | owner #1 |
| `\dotso` | TODO | owner #1 |
...
| `{drcases}` | accept | goldens: aligned, alignedat, array |
...
| `\end` | accept | goldens: aligned, alignedat, array |
...
| `\frac` | accept | goldens: bigl, demo-cfrac, demo-sumsq |
| `\frak` | TODO | owner #1 |
...
| `{gathered}` | accept | goldens: aligned, alignedat, array |
...
| `\hArr` | TODO | owner #1 |
| `\harr` | TODO | owner #1 |
| `\hat` | accept | goldens: demo-fourier, hat, sym-greek |
| `\hbar` | accept | goldens: sym-gal-4 |
| `\hbox` | TODO | owner #7 |
| `\hbox to` | unsup | KaTeX rejects too; no reject row yet (owner #7) |
| `\hdashline` | accept | goldens: aligned, alignedat, array |
| `\hearts` | TODO | owner #1 |
...
| `\hline` | accept | goldens: aligned, alignedat, array |
...
| `\injlim` | TODO | owner #1 |
| `\int` | accept | goldens: demo-fourier, demo-gauss, int |
...
| `\left` | accept | goldens: demo-cauchy, dfrac, leftright |
...
| `\mathchoice` | accept | goldens: demo-fourier, int, int-display |
| `\mathclap` | accept | goldens: demo-cauchy, demo-sumsq, displaystyle |
...
| `\mathstrut` | accept | goldens: demo-cfrac, demo-gauss, sqrt |
...
| `{matrix}` | accept | goldens: aligned, alignedat, array |
| `{matrix\*}` | accept | goldens: aligned, alignedat, array |
...
| `\middle` | accept | goldens: demo-cauchy, leftright, leftright-dot |
...
| `\newcommand` | accept | goldens: color-macro, newcommand, newcommand-arg |
...
| `\nolimits` | accept | goldens: lim, lim-display, nolimits |
...
| `\operatorname\*` | accept | goldens: limits-force, operatorname, operatorname-star |
...
| `\overbrace` | accept | goldens: braces, text |
...
| `\overparen` | unsup | KaTeX rejects too; no reject row yet (owner #1) |
...
| `{pmatrix}` | accept | goldens: aligned, alignedat, array |
| `{pmatrix\*}` | accept | goldens: aligned, alignedat, array |
...
| `\raise` | unsup | KaTeX rejects too; no reject row yet (owner #7) |
...
| `\rbrace` | TODO | owner #1 |
| `\rbrack` | TODO | owner #1 |
| `{rcases}` | accept | goldens: aligned, alignedat, array |
...
| `\relax` | TODO | owner #1 |
| `\renewcommand` | accept | goldens: def, def-args, renewcommand |
...
| `\right` | accept | goldens: demo-cauchy, dfrac, leftright |
...
| `\scriptscriptstyle` | accept | goldens: bigl, demo-cfrac, demo-sumsq |
| `\scriptsize` | TODO | owner #1 |
| `\scriptstyle` | accept | goldens: bigl, demo-cfrac, demo-sumsq |
| `\sdot` | TODO | owner #1 |
| `\searrow` | accept | goldens: sym-gal-7 |
| `\sec` | accept | goldens: sym-gal-7 |
| `\sect` | accept | goldens: text |
| `\set` | TODO | owner #1 |
| `\Set` | accept | goldens: bigl, demo-cfrac, demo-sumsq |
...
| `{smallmatrix}` | accept | goldens: aligned, alignedat, array |
| `\smallsetminus` | TODO | owner #1 |
| `\smallsmile` | TODO | owner #1 |
| `\smash` | accept | goldens: demo-cauchy, leftright, leftright-dot |
...
| `\square` | TODO | owner #1 |
| `\sqrt` | accept | goldens: demo-cfrac, demo-gauss, sqrt |
...
| `\substack` | accept | goldens: demo-cauchy, demo-sumsq, displaystyle |
...
| `\sum` | accept | goldens: demo-cauchy, demo-sumsq, displaystyle |
...
| `\tbinom` | TODO | owner #2 |
| `\TeX` | TODO | owner #1 |
| `\text` | accept | goldens: sym-escapes, text |
...
| `\textregistered` | accept | goldens: sym-gal-8, text |
...
| `\textstyle` | accept | goldens: demo-cauchy, demo-sumsq, displaystyle |
...
| `\underbrace` | accept | goldens: braces, text |
...
| `\vcenter` | accept | goldens: bigl, demo-cauchy, demo-cfrac |
| `\vcenter` | accept | goldens: bigl, demo-cauchy, demo-cfrac |
| `\Vdash` | TODO | owner #1 |
| `\vDash` | TODO | owner #1 |
...
| `\veebar` | TODO | owner #1 |
| `\verb` | accept | goldens: bigl, demo-cfrac, demo-sumsq |
...
---
Totals: 550 accept, 4 err-parity, 123 unsup, 463 TODO.
